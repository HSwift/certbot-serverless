...
完整的首次部署、Secrets 初始化、域名、Zero Trust、验证和故障排查步骤见 [DEPLOYMENT.md](./DEPLOYMENT.md)。本节保留配置摘要；以独立部署文档为准。
...
Deploy command 会执行 D1 migration。Cloudflare 默认自动生成 Workers Builds 的部署 Token，通常直接使用默认 Token 即可。只有构建日志明确报告权限不足时，才选择具备以下范围的自定义 Token：
...
自定义 Token 可在 **Settings → Builds → API token** 中选择。这个部署 Token 与 `RUNTIME_CLOUDFLARE_API_TOKEN` 是两个不同用途的 Token。
...
1. 在 **Workers & Pages → certbot-serverless-console → Access** 中选择 **Protect this Worker behind Access**，保护所有流量。
2. 创建 Allow Policy，只包含你的精确邮箱或明确的 IdP 用户组；不要使用 `Include Everyone`。
3. 从生成的 Console Access Application 复制 Audience Tag，写入 API Build 的 `ACCESS_AUD`。
4. 将完整 Team Domain 写入 API Build 的 `ACCESS_TEAM_DOMAIN`，重新部署 API Worker。
5. 访问 Console，确认 `/api/me` 返回 `method: access`。
6. API Worker 自定义域名保持可路由，由 API Worker 对 `/api/*` 验证 Access JWT 或 Bearer，对 `/download/*` 验证预签名。
